...
public class UserRestController {
	...
	@PostMapping("/login")
	private ResponseEntity<Void> logIn(@Valid @RequestBody UserDto.LoginRequest loginRequest){
	...
	private ResponseEntity<UserDto.UserInfoResponse> findMyInfo(@CurrentUser String email){
		return ResponseEntity.ok().body(sessionLoginService.currentUser(email));
	}

	@LoginCheck
	@GetMapping("/logout")
	private ResponseEntity<Void> logOut(@CurrentUser String email){
		sessionLoginService.logOut(email);
		return OK;
	}
	...
	@LoginCheck
	@DeleteMapping
	private ResponseEntity<Void> deleteByEmail(@CurrentUser String email){
		sessionLoginService.logOut(email);
